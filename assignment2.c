...
Description:	Lotto Program

                This program emulates the lotto. The program has
                many functions:

                1. It takes a select amount of numbers from the
                user. It validates them and then places them into
                an array. This must be done first before any other
                function can be called. This can be done an 
                unlimited amout of times before the program is ended.

                2. It displays the lotto numbers that are stored
                in the array. It displays them in the order that 
                they are input if they are not sorted.

                3. It allows the user to sort the array using the
                bubble sort algorithm.

                4. It compares the user's lotto numbers to the lotto
                numbers that are picked as the winning numbers. These
                numbers are hard coded and not randomly generated.
                Depending on the amount of matching numbers, a
                different message is displayed stating the prize that
                has been won by the user.

                5. It displays the frequency of the numbers that are
                entered by the user. The frequency accumulates as more
                and more numbers are entered. Only numbers that appear
                at least one time are displayed. The frequency is set 
                to zero upon running the program again.

                6. It exits the game and ends the program.

Author:			Rokas Galinauskas
Date:			08/03/2022
